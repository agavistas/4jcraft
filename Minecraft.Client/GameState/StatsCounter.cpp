...
void StatsCounter::WipeLeaderboards() {
    ...
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_KILLS_EASY)
        XUserResetStatsViewAllUsers(STATS_VIEW_KILLS_EASY, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_KILLS_NORMAL)
        XUserResetStatsViewAllUsers(STATS_VIEW_KILLS_NORMAL, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_KILLS_HARD)
        XUserResetStatsViewAllUsers(STATS_VIEW_KILLS_HARD, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGBLOCKS_PEACEFUL)
        XUserResetStatsViewAllUsers(STATS_VIEW_MINING_BLOCKS_PEACEFUL, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGBLOCKS_EASY)
        XUserResetStatsViewAllUsers(STATS_VIEW_MINING_BLOCKS_EASY, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGBLOCKS_NORMAL)
        XUserResetStatsViewAllUsers(STATS_VIEW_MINING_BLOCKS_NORMAL, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGBLOCKS_HARD)
        XUserResetStatsViewAllUsers(STATS_VIEW_MINING_BLOCKS_HARD, nullptr);
    // 	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGORE_PEACEFUL )
    // XUserResetStatsViewAllUsers(STATS_VIEW_MINING_ORE_PEACEFUL, nullptr);
    // if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGORE_EASY )
    // XUserResetStatsViewAllUsers(STATS_VIEW_MINING_ORE_EASY, nullptr); 	if(
    // DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGORE_NORMAL )
    // XUserResetStatsViewAllUsers(STATS_VIEW_MINING_ORE_NORMAL, nullptr);
    // if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_MININGORE_HARD )
    // XUserResetStatsViewAllUsers(STATS_VIEW_MINING_ORE_HARD, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_FARMING_PEACEFUL)
        XUserResetStatsViewAllUsers(STATS_VIEW_FARMING_PEACEFUL, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_FARMING_EASY)
        XUserResetStatsViewAllUsers(STATS_VIEW_FARMING_EASY, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_FARMING_NORMAL)
        XUserResetStatsViewAllUsers(STATS_VIEW_FARMING_NORMAL, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_FARMING_HARD)
        XUserResetStatsViewAllUsers(STATS_VIEW_FARMING_HARD, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_TRAVELLING_PEACEFUL)
        XUserResetStatsViewAllUsers(STATS_VIEW_TRAVELLING_PEACEFUL, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_TRAVELLING_EASY)
        XUserResetStatsViewAllUsers(STATS_VIEW_TRAVELLING_EASY, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_TRAVELLING_NORMAL)
        XUserResetStatsViewAllUsers(STATS_VIEW_TRAVELLING_NORMAL, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_TRAVELLING_HARD)
        XUserResetStatsViewAllUsers(STATS_VIEW_TRAVELLING_HARD, nullptr);
    // 	if( DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_NETHER_PEACEFUL )
    // XUserResetStatsViewAllUsers(STATS_VIEW_NETHER_PEACEFUL, nullptr); 	if(
    // DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_NETHER_EASY )
    // XUserResetStatsViewAllUsers(STATS_VIEW_NETHER_EASY, nullptr); 	if(
    // DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_NETHER_NORMAL )
    // XUserResetStatsViewAllUsers(STATS_VIEW_NETHER_NORMAL, nullptr); 	if(
    // DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_NETHER_HARD )
    // XUserResetStatsViewAllUsers(STATS_VIEW_NETHER_HARD, nullptr);
    if (DEBUG_CLEAR_LEADERBOARDS & LEADERBOARD_TRAVELLING_TOTAL)
        XUserResetStatsViewAllUsers(STATS_VIEW_TRAVELLING_TOTAL, nullptr);
    if (LeaderboardManager::Instance()->OpenSession()) {
        writeStats();
        LeaderboardManager::Instance()->CloseSession();
    }
